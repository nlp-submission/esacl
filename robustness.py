...
parser.add_argument("--bart_file", type=str, default='./output/finetune-distilbart-xsum/test_generations.txt')
parser.add_argument("--pred_file", type=str, default='./output/ri-ri/test_generations.txt')
...
def distillation(preds, articles, targets, rouge_key='rouge1', percentile=0.1):
    ...
    for i in range(len(articles)):
        ...
        metrics = calculate_rouge(pred_lns=[target], tgt_lns=[article], rouge_keys=[rouge_key],
                                  return_precision_and_recall=True)
        result.append(
            {'article': article, 'pred': pred, 'target': target, 'distill': 1 - metrics[rouge_key]['recall']})
    ...
    # calculate the total number of observations in two slices
    print(f"There are {len(shortest_article)} observations in Least Distilled")
    print(f"There are {len(longest_article)} observations in Most Distilled")
...
for i in range(2):
    if i < 1:
        ...
        print(f"Robustness Result for ELSA")
